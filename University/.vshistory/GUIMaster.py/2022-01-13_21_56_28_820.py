...
class chuck_norris_jokes(Frame):
 ...
 def __init__(self, parent, controller):
    ...
    button_text=[]
    btn_name=(button_text[n])

    for x in range(len(joke_menu)):
        btn_text=joke_menu[x]
      
        button=Button(self, text=btn_text, command = partial(change,x))
        b.pack()
    ...
    #    b=Button(self, text=btn_text, command = lambda:[games_call.chuck_jokes.display_joke(controller,),controller.show_frame(chuck_norris_jokes)])
    #    b.pack()
    #    button.append(b)
