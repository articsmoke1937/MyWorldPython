...
class chuck_norris_jokes(Frame):
 ...
 def __init__(self, parent, controller):
    ...
    for x in range(len(joke_menu)):
        ...
        print(btn_text'assign')
        ##b=Button(self, text=t, command = lambda x = x: boardWindow(x))
        b=Button(self, text=btn_text, command = lambda:[games_call.chuck_jokes.display_joke(controller,btn_text),controller.show_frame(chuck_norris_jokes)])
        b.pack()
        button.append(b)
